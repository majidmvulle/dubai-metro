...
#define UPDATE_PROXIMITY_INTERVAL 5.0

@interface DMViewController () <CLLocationManagerDelegate, DMLocationManagerDelegate>
...
- (void)viewDidLoad
{
    ...
    [self.dashboardVC.startJourneyButton addTarget:self action:@selector(startJourney)
                                  forControlEvents:UIControlEventTouchDown];
...
{
    if([segue.identifier isEqualToString:@"Embed Dashboard"]){
    ...
    }else if([segue.identifier isEqualToString:@"Embed Map View"]){
        if([segue.destinationViewController isKindOfClass:[AllMetroStationsMapViewController class]]){
            ...
            self.metroStationsMapvc.managedObjectContext = self.managedObjectContext;
        }
    ...
    if (![coreDataHelper.fileManager fileExistsAtPath:[coreDataHelper.managedDocument.fileURL path]]){
        ...
        [self.managedDocument openWithCompletionHandler:^(BOOL success) {
            if (success) {
                ...
                if(![self coreDataHasEntriesForEntityName:@"MetroStation"]) [self populateMetroStations];
            }
        }];
    } else {
        self.managedObjectContext = self.managedDocument.managedObjectContext;
...
- (void)reloadMap
{
    if(self.view.window){
        ...
        [self.metroStationsMapvc.mapView setCenterCoordinate:self.metroStationsMapvc.mapView.userLocation.coordinate animated:YES];
    }
}
...
- (void)authorizationStatusChangedTo:(CLAuthorizationStatus)status
{
    ...
    else self.dashboardVC.status = kLocationOff;
}
